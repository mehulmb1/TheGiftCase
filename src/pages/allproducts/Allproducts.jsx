...
function Allproducts() {
  ...
  const { mode, product ,searchkey, setSearchkey,filterType,setFilterType,
      filterPrice,setFilterPrice,addtocartfirebase} = context
    ...
    <Layout>
      ...
      <section className="text-gray-600 body-font">
        ...
        {uniqueCategories.map(category => (
            <div className="container px-5 py-8 md:py-16 mx-auto" key={category}>
                <div className="lg:w-1/2 w-full mb-6 lg:mb-10">
                    <h1 className="sm:text-3xl text-2xl font-medium title-font mb-2 text-gray-900" style={{ color: mode === 'dark' ? 'white' : '' }}>{category}</h1>
                    <div className="h-1 w-20 bg-pink-600 rounded"></div>
                </div>
                <div className="flex flex-wrap -m-4">
                    {product.filter( (obj)=> obj.title.toLowerCase().includes(searchkey))
                     .filter((obj) => obj.category.includes(category))
